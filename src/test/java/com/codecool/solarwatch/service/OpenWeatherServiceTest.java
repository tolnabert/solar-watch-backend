...
class OpenWeatherServiceTest {
    ...
    void testGetLocation_LocationOfBudapest_BothShouldBeTrue() {
        ...
        when(restTemplate.getForObject(any(String.class), eq(LocationReport[].class))).thenReturn(mockResponse);

        List<LocationReport> locations = openWeatherService.getLocation("Budapest", 1);

        assertEquals(1, locations.size());

        LocationReport location = locations.get(0);
        assertEquals(47.4979937, location.lat());
        assertEquals(19.0403594, location.lon());
    }

    @Test
    void testGetTwilight_TwilightOfBudapest_BothShouldReturnTrue() {
        TwilightResponse mockResponse = new TwilightResponse(new TwilightReport("4:09:54 AM", "5:22:19 PM"));
        ...
        TwilightReport results = openWeatherService.getTwilight(location);
    ...
    void testGetLocation_NoLocationDataFound() {
        ...
        RuntimeException exception = assertThrows(RuntimeException.class,
                () -> openWeatherService.getLocation("UnknownCity", 1));
